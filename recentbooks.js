...
fetch(url)
  ...
  .then((data) => {
    ...
    let startDiv = item.map(function (item) {
      ...
      return (
        `<div class = "allrecent"><div class="recentimg">
    ...
    for (let i = 0; i < startDiv.length; i++) {
      ...
      let display = (document.getElementById("recent1").innerHTML =
        startDiv[i]);
      ...
      new Promise((resolve) =>
        setTimeout(() => resolve(display), timeInterval)
      ).then(function () {
      ...
      console.log(
        new Promise((resolve) =>
          setInterval(() => resolve(display), timeInterval)
        )
      );
...
fetch(url2)
  ...
  .then((data) => {
    ...
    let displayResult = item.map(function (item) {
      ...
      console.log(volumeInfo.publisher);
      return (
        `<div class = "allrecent2"><div class="recentinfo2"><div class="head2"><p class="allheadings2">Title:</p>
        ${volumeInfo.title}</div>
        ...
        <div class="head2"><p class="allheadings2">Preview on Google:</p><a href="${volumeInfo.previewLink}">Google Books</a> </div>` +
        `</div></div>`
      );
    });
    let timeInterval = 700;
    ...
    for (let i = 0; i < displayResult.length; i++) {
      ...
      let display = (document.getElementById("freebooks").innerHTML =
        displayResult[i]);
